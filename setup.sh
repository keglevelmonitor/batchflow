#!/bin/bash
# setup.sh
# Single-line installer wrapper for BatchFlow

# 1. Define the Install Directories
INSTALL_DIR="$HOME/batchflow"

# --- Export variables so install.sh can see them ---
export DATA_DIR="$HOME/batchflow-data"
export DESKTOP_FILENAME="batchflow.desktop"
export APP_TITLE="BatchFlow"
# ---------------------------------------------------

WHAT_TO_INSTALL="BatchFlow Application and Data Directory"
CLEANUP_MODE="NONE"

echo "========================================"
echo "       BatchFlow Auto-Installer"
echo "========================================"
...
if [ -d "$INSTALL_DIR" ] || [ -d "$DATA_DIR" ]; then
    ...
    if [ "$choice" == "APP" ]; then
        WHAT_TO_INSTALL="BatchFlow Application"
        CLEANUP_MODE="APP"
...
if [ -d "$INSTALL_DIR" ]; then
    ...
    # UPDATED: Pointing to the assumed batchflow repo
    git clone https://github.com/keglevelmonitor/batchflow.git "$INSTALL_DIR"
    cd "$INSTALL_DIR" || exit 1
fi
